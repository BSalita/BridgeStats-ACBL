...
BRIDGESTATS_API_BASE_URL = os.environ.get(
    "BRIDGESTATS_API_BASE_URL",
    os.environ.get("ACBL_STATS_API_BASE_URL", "http://127.0.0.1:8523"),
).rstrip("/")
_TIMEOUT_S = 300
...
def _request(
    ...
    try:
        ...
        raise BridgeStatsApiClientError(
            f"BridgeStats API unreachable at {BRIDGESTATS_API_BASE_URL}: {exc}"
        ) from exc
    if not resp.ok:
        try:
            body = resp.json()
...
def dataset_info() -> Dict[str, Any]:
    return _request("GET", "/acbl-stats/dataset-info")
...
def favorites(favorite_id: Optional[str] = None) -> Dict[str, Any]:
    return _request("GET", "/acbl-stats/favorites", {"id": favorite_id})
...
def run_favorite(
    ...
    return _request(
        "POST",
        f"/acbl-stats/favorites/{favorite_id}",
        json={
            "source": source,
            "meta": meta or {},
            "limit": limit,
